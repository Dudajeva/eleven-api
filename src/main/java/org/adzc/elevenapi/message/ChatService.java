...
public class ChatService {
    ...
    public List<MessageDTO> listMessages(Long me, Long cid, Long cursor, int size) {
        ensureMember(me, cid);
        var list = chatMessageMapper.pageDesc(cid, cursor, Math.min(size, 50));
        return list.stream().map(m -> new MessageDTO(
                m.getId(), m.getConversationId(), m.getSenderId(),
                m.getMessageType(), m.getContent(), m.getImageUrl(), m.getCreatedAt()
        )).toList();
    }
    ...
    public Saved saveMessage(Long senderId, Long conversationId, String msg, String imgUrl) {
        ...
        chatMessageMapper.insertSelective(m); // 自增
    ...
    public int calcUnread(Long uid, Long conversationId) {
        ...
        return chatMessageMapper.countUnread(conversationId, uid, cursor);
    }
    ...
    public Long openOrCreate(Long me, Long targetId) {
        ...
        chatConversation.setChatType(1);//私聊
        chatConversationMapper.insertSelective(chatConversation);

        ChatMember chatMemberU1 = new ChatMember();
        chatMemberU1.setUserId(u1);
        chatMemberU1.setConversationId(chatConversation.getId());
        chatMemberMapper.insertSelective(chatMemberU1);

        ChatMember chatMemberU2 = new ChatMember();
        chatMemberU2.setUserId(u2);
        chatMemberU2.setConversationId(chatConversation.getId());
        chatMemberMapper.insertSelective(chatMemberU2);
